...
check_openstar(){
    ...
    if [[ -f ${install_path}/nginx/sbin/nginx ]]; then
        or_status=`nohup ${install_path}/nginx/sbin/nginx -v >/tmp/1 2>&1 && grep -oE "[0-9.]+" /tmp/1`
        echo -e " openresty  版本: ${Green_font_prefix}${or_status}${Font_color_suffix}"
    else
        echo -e " openresty  状态: ${Red_font_prefix}未安装${Font_color_suffix}${Red_font_prefix}请先安装 openresty ${Font_color_suffix}"
    fi
...
function up_openstar(){
    ...
    mv ${install_path}/openstar ${install_path}/openstar.bak
    cd ${install_path}
    git clone https://github.com/op-sec-team/releases-openstar-Enterprise
...
function up_view(){
    ...
    rm -rf ${install_path}/releases-openstar-Enterprise
    mv ${install_path}/nginx/html/view-private ${install_path}/nginx/html/view-private.bak
    cd ${install_path}
    git clone https://github.com/op-sec-team/releases-openstar-Enterprise
